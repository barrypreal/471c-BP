...
def cps_convert_term(
    ...
    match term:
            ...
            return L1.Abstract(
                ...
                then=_term(
                    ...
                    lambda target: _terms(
                        arguments,
                        lambda arguments: L1.Apply(target=target, arguments=[*arguments, k]),
                    ),
                ),
            )
        ...
        case L2.Immediate(value=value):
            tmp = fresh("t")
            return L1.Immediate(destination=tmp, value=value, then=m(tmp))
            ...
            return _term(
                ...
                m=lambda left: _term(
                    ...
                    m=lambda right: L1.Primitive(
                        destination=tmp, operator=operator, left=left, right=right, then=m(tmp)
                    ),
                ),
            )
                ...
                then=_term(
                    ...
                    lambda left: _term(
                        ...
                        lambda right: L1.Branch(
                            ...
                            left=left,
                            right=right,
                            then=_term(consequent, lambda consequent: L1.Apply(target=tmpJ, arguments=[consequent])),
                            otherwise=_term(otherwise, lambda otherwise: L1.Apply(target=tmpJ, arguments=[otherwise])),
                        ),
                    ),
                ),
            )
