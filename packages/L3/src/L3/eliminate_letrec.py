...
def eliminate_letrec_term(
    ...
    match term:
        ...
        case L3.Apply(target=target, arguments=arguments):
            newArguments = [recur(argument) for argument in arguments]
            return L2.Apply(target=recur(target), arguments=newArguments)
        ...
        case L3.Store(base=base, index=_index, value=value):
            return L2.Store(base=recur(base), index=0, value=recur(value))
